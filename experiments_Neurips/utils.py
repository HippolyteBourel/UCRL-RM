from environments import gridworld, discreteMDP, MDPRM_library
import pylab as pl
import gym
import pickle
...
def buildGridworld_RM(sizeX=11,sizeY=9,map_name="2-room_1corner",rewardStd=0., initialSingleStateDistribution=False,max_steps=np.infty,reward_threshold=np.infty):
	register(
		id='Gridworld-RM'+map_name+'-v0',
		entry_point='environments.MDPRM_library:GridWorld_RM',
		max_episode_steps=max_steps,
		reward_threshold=reward_threshold,
		kwargs={'sizeX': sizeX,'sizeY':sizeY,'map_name':map_name,'rewardStd':rewardStd, 'initialSingleStateDistribution':initialSingleStateDistribution}
	)
...
# Alternative function to use with MDPRM environment (and RM adapted algorithm)
def cumulativeRewards_RM(env,learner,nbReplicates,timeHorizon,rendermode='pylab', reverse = False):
	cumRewards = []
	if(rendermode==''):
		for i_episode in range(nbReplicates):
			observation = env.reset()
			...
			print("New initialization of ", learner.name())
			print("Initial state:" + str(observation))
			print("Initial state of RM:" + str(env.rewardMachine.current_state))
			for t in range(timeHorizon):
				state = observation
				action = learner.play(state)  # Get action
				if reverse:
...
# Value iteration
def value_iter(env, gamma = 1, epsilon = 0.01, mean_reward = np.array([None]), max_iter = 100000):
	if mean_reward.any() == None:
		mean_reward = env.mean_reward
	niter = 0
	policy = np.zeros(env.nS, dtype=int)
	if gamma < 1:
		u0 = np.array([1/(1 - gamma) for _ in range(env.nS)])
	...
	while True:
		...
		for s in range(env.nS):
			for a in range(env.nA):
				temp = mean_reward[s, a] + gamma * sum([u * p for (u, p) in zip(u0, P[s, a])])
				if (a == 0) or (temp > u1[s]):
					u1[s] = temp
					policy[s] = a
		diff  = [abs(x - y) for (x, y) in zip(u1, u0)]
		if (max(diff) - min(diff)) < epsilon:
			return niter, u0, policy
		else:
			u0 = u1
			u1 = np.zeros(env.nS)
		if niter > max_iter:
...
def compute_gstar(env, epsilon = 0.00001, max_iter = 100000):
		...
		for s in range(env.nS):
			for a in range(env.nA):
				...
				if (a == 0) or (temp > u1[s]):
					u1[s] = temp
					policy[s] = a
		diff  = abs(u1[0]/(niter - 1) - u0[0]/niter)
		if diff < epsilon:
			return u0[0]/niter
		else:
			u0 = u1
...
def compute_gstar_samdp(env, epsilon = 0.00001, max_iter = 100000):
	gain = 0
	RM = env.rewardMachine
	nQ = RM.nb_states
	u0 = np.zeros((nQ, env.nS), dtype=float)
	u1 = np.zeros((nQ, env.nS), dtype=float)
		...
		for q in range(nQ):
			for s in range(env.nS):
				for a in range(env.nA):
					event = RM.events[s, a]
					if event != None:
						reward = RM.rewards[q, event]
					else:
						reward = 0
					temp = reward
					for qq in range(nQ):
						for ss in range(env.nS):
							temp += u0[qq, ss] * P[q, s, a, qq, ss]
					if (a == 0) or (temp > u1[q, s]):
						u1[q, s] = temp
		diff = u1[0, 0]/(niter - 1) - u0[0, 0]/niter
		if diff < epsilon:
			gain = u0[0, 0]/niter
			break
		else:
